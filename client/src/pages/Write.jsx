...
const Write = () => {
  ...
  const upload = async()=>{
    try {
      ...
      const res =await axios.post('http://localhost:3005/api/upload/',formData)
     return res.data
    } catch (err) {
      console.log(err)
    }
...
const handleClick = async(e)=>{
 ...
 try {
  state ? await axios.put(`http://localhost:3005/api/posts/${state.id}`,{
    title,desc:value,cat,img:file? imgUrl:""
  },{
  withCredentials: true,
}) : 
   await axios.post(`http://localhost:3005/api/posts`,{
    title,desc:value,cat,img:file? imgUrl:"",
